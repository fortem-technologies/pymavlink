 ...
 * can also be used if the message header has already been written before (as in
 * mavlink_msg_xxx_pack instead of mavlink_msg_xxx_pack_headerless), it just introduces little extra
 * overhead.
 *
...
#endif
{
    ...
    // Use an additional variable to avoid taking the address of a member of a packed struct.
    uint16_t chksm = msg->checksum;
    crc_accumulate_buffer(&chksm, _MAV_PAYLOAD(msg), msg->len);
    msg->checksum = chksm;
#if MAVLINK_CRC_EXTRA
    crc_accumulate(crc_extra, &chksm);
    msg->checksum = chksm;
#endif
...
MAVLINK_HELPER uint16_t mavlink_finalize_message(
    ...
    return mavlink_finalize_message_chan(
        msg, system_id, component_id, MAVLINK_COMM_0, min_length, length, crc_extra);
}
#else
MAVLINK_HELPER uint16_t mavlink_finalize_message(
    mavlink_message_t* msg,
 ...
 *                 A channel is not a physical message channel like a serial port, but a logical
 * partition of the communication streams. COMM_NB is the limit for the number of channels on MCU
 * (e.g. ARM7), while COMM_NB_HIGH is the limit for the number of channels in Linux/Windows
 * @param c        The char to parse
 *
 ...
 *     {
 *     printf("Received message with ID %d, sequence: %d from component %d of system %d", msg.msgid,
 ...
 * @endcode
 */
MAVLINK_HELPER uint8_t mavlink_frame_char(
    uint8_t chan,
    uint8_t c,
    mavlink_message_t* r_message,
    mavlink_status_t* r_mavlink_status) {
 ...
 */
MAVLINK_HELPER uint8_t put_bitfield_n_by_index(
    int32_t b,
    uint8_t bits,
